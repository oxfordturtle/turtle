import type { Type } from "@/core/compiler.ts";
...
export interface Command {
  ...
  // Python list support
  readonly listOperand?: "lp" | "size"; // the encoder appends this operand itself
  readonly forList?: boolean; // disambiguates a colliding dot-method name (".index") when the receiver is a list
  readonly returnsList?: boolean; // true only for ".copy" - the result is a list of the same element kind as the receiver, not the (placeholder) scalar `returns` type
  readonly listBehavior?:
    | "append"
    | "insert"
    | "extend"
    | "remove"
    ...
    | "index"
    | "copy"
    | "length"; // triggers custom encoding beyond the generic listOperand append - see encoder/lists.ts
}
...
const command = (
  ...
  listExtras?: Pick<
    Command,
    "listOperand" | "forList" | "returnsList" | "listBehavior"
  >,
): Command => {
  const id =
    typeof name === "string"
      ? name
      : name.find((n): n is string => typeof n === "string")!;
...
export interface Parameter {
  ...
  // Python list support: `type` stays strictly scalar, so these two flags carry
  // what it can't express
  readonly isList?: boolean; // this parameter must itself be a list value (a method's receiver, or ".extend"'s argument)
  readonly matchesListElement?: boolean; // this parameter's expected scalar type is the *receiver* list's element kind, not a fixed Type
}
...
export const commands = [
  ...
  command(
    ...
    // no dedicated opcode: like the Delphi compiler, SQR is ROOT with a constant
    // 2 for the degree - push 2, then swap it below the already-pushed mult so
    // the stack reads (a, 2, mult)
    () => [PCode.ldin, 2, PCode.swap, PCode.root],
  ...
  command(
    ...
    "integer",
    { category: 5, level: 2 },
    "Returns <code>(a/b)<sup>c/d</sup></code>, multiplied by <code>mult</code> and rounded to the nearest integer. Use <code>c</code>/<code>d</code> to approximate a fractional exponent, and <code>mult</code> to approximate real numbers.",
  ),
  command(
    "root",
    () => [PCode.root],
    ...
    "integer",
    { category: 5, level: 2 },
    "Returns <code><sup>b</sup>&radic;a</code>, multiplied by <code>mult</code> and rounded to the nearest integer. Use the multiplier to approximate real numbers.",
  ),
  command(
    "lerp",
    () => [PCode.lerp],
    ...
    "integer",
    { category: 5, level: 2 },
    "Returns a linear interpolation between <code>a</code> and <code>b</code>, at position <code>t</code>/<code>scale</code> along the way (so <code>t=0</code> gives <code>a</code> and <code>t=scale</code> gives <code>b</code>), rounded to the nearest integer.",
  ),
  command(
    "divMult",
    () => [PCode.divm],
  ...
  command(
    ...
    "integer",
    { category: 7, level: 1 },
    "Returns the length of the input <code>string</code> (i.e. the number of characters, or - for a Python list argument - the number of elements).",
    { listBehavior: "length" },
  ),
  command(
    [, , , ".length"],
  ...
  command(
    [, , , , ".find"],
    // SWAP because POSS takes its arguments the other way round; DECR because
    // Python's str.find() is 0-based with -1 for not found, and POSS is 1-based
    // with 0
    () => [PCode.swap, PCode.poss, PCode.decr],
    [p("string", "string"), p("substr", "string")],
    "integer",
    { category: 7, level: 2 },
    "Searches for the input <code>substring</code> within the given <code>string</code>; returns the 0-based index of the first character if found, -1 otherwise.",
  ),
  command(
    [, , , "pos"],
    () => [PCode.poss],
  ...
  command(
    [, , , , ".append"],
    () => [PCode.lapp],
    [pList("list"), pElement("object")],
    null,
    { category: 7, level: 1 },
    "Appends the object to the specified list.",
    { listOperand: "lp", listBehavior: "append" },
  ),
  command(
    [, , , , ".copy"],
    () => [PCode.lcpy],
    [pList("list")],
    "boolint", // placeholder - the real result type is a list of the receiver's element kind, see returnsList
    { category: 7, level: 1 },
    "Returns a copy of the specified list.",
    { listOperand: "lp", returnsList: true, listBehavior: "copy" },
  ),
  command(
    [, , , , ".extend"],
    () => [PCode.lext],
    [pList("list"), pList("other")],
    null,
    { category: 7, level: 1 },
    "Extends the contents of the specified list by concatenation.",
    { listOperand: "lp", listBehavior: "extend" },
  ),
  command(
    [, , , , ".insert"],
    () => [PCode.lins],
    [pList("list"), p("index", "integer"), pElement("object")],
    null,
    { category: 7, level: 1 },
    "Inserts the object at the specified index.",
    { listOperand: "lp", listBehavior: "insert" },
  ),
  command(
    [, , , , ".remove"],
    () => [PCode.lrem],
    [pList("list"), pElement("value")],
    null,
    { category: 7, level: 1 },
    "Removes the first item from the specified list whose value is equal to the specified value.",
    { listOperand: "lp", listBehavior: "remove" },
  ),
  command(
    [, , , , ".del"],
    () => [PCode.ldel],
    [pList("list"), p("index", "integer")],
    null,
    { category: 7, level: 1 },
    "Deletes the item at the specified index from the specified list.",
    { listOperand: "lp", listBehavior: "del" },
  ),
  command(
    [, , , , ".reverse"],
    () => [PCode.lrev],
  ...
  command(
    ...
    "integer",
    { category: 7, level: 1 },
    "Searches for the specified value within the specified list; returns the index of the first occurrence if found, raises an error otherwise.",
    { listOperand: "lp", forList: true, listBehavior: "index" },
  ),
  ...
  command(
    ["PRINT#", "fwrite", "fwrite", "fwrite", "fwrite", "fwrite"],
    () => [PCode.fwrs],
  ...
  command(
    ["PRINTLN#", "fwriteln", "fwriteln", "fwriteln", "fwriteline", "fwriteln"],
    () => [PCode.fwln],
    [p("file handle", "integer"), p("string", "string")],
    null,
    { category: 10, level: 1 },
  ...
  command(
    ["FINDDIR$", "findDir", "findDir", "finddir", "finddir", "findDir"],
    () => [
      PCode.dupl,
      PCode.lptr,
      PCode.rota,
    ...
    [p("directory name pattern", "string"), p("file handle", "integer", true)],
    "string",
    { category: 10, level: 2 },
  ...
  command(
    ...
    [p("filename pattern", "string"), p("file handle", "integer", true)],
    "string",
    { category: 10, level: 2 },
    "Finds the first file matching the pattern.",
  ),
  command(
    ["FINDNEXT$", "findNext", "findNext", "findnext", "findnext", "findNext"],
    () => [PCode.fnxt],
    [p("file handle", "integer")],
    "string",
    { category: 10, level: 2 },
