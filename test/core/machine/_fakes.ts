 ...
 * real flood-fill algorithm (that stays adapter-side, out of scope for this
 * plan) - it just records the call and sets the single seed pixel, enough
 * to exercise the machine's call construction.
 */
 ...
 *   order - this *is* the machine's event system (see plan doc).
 ...
 * callbacks, not promise continuations (see 04's "Testing async PCode
 * handlers" note for the pattern steps 05/06 should use).
 *
 ...
 * segments - per 04's design, that sandboxing check is `runtime.ts`'s job,
 * not the port's, so the fake mirrors a real backend in staying agnostic
 * to it.
 */
